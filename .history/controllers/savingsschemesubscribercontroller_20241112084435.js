...
exports.getSchemeDetails = async (req, res) => {
  ...
  try {
    ...
    const subscriber = await SavingsSchemeSubscriber.findById(subscriberId)
      ...
      .populate('address')
      .populate('scheme', 'name duration duedate');
    ...
    if (subscriber.scheme) {
      schemeName: subscriber.scheme.name, // Access scheme name if scheme exists
    } else {
    ...
    res.status(200).json({
      ...
      schemeDetails: {
        ...
        dueDate: subscriber.nextDueDate, // Use 'duedate' as the field name from Scheme
