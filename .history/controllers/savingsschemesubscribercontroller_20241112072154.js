...
exports.getSchemeDetails = async (req, res) => {
  ...
  try {
    ...
    const subscriber = await SavingsSchemeSubscriber.findById(subscriberId)
      ...
      .populate('address')
      .populate('scheme', 'name duration nextDueDate'); // Populate scheme details (fields name, duration, nextDueDate)
      ;
    ;
    if (!subscriber) {
    ...
    res.status(200).json({
      ...
      schemeDetails: {
        schemeName: subscriber.scheme ? subscriber.scheme.name : undefined,  // Access scheme name if populated
        amount: subscriber.amount,
        subscribedDate: subscriber.subscribedDate,
        nextDueDate: subscriber.scheme ? subscriber.scheme.nextDueDate : undefined, // Access nextDueDate if populated
        duration: subscriber.scheme ? subscriber.scheme.duration : undefined, // Access duration if populated
