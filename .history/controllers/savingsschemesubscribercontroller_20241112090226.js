...
exports.createSavingsSchemeSubscriber = async (req, res) => {
  ...
  try {
    ...
    const savingsschemesubscriber = await SavingsSchemeSubscriber.create({
      ...
      subscribedDate: new Date(),  // Current date as the subscribed date
      nextDueDate: scheme.duedate,  // Use the due date from the scheme
      duration: scheme.duration     // Store the duration
    })
...
exports.getSchemeDetails = async (req, res) => {
  ...
  try {
    ...
    const subscriber = await SavingsSchemeSubscriber.findById(subscriberId)
      ...
      .populate({
        path: 'scheme',
        select: 'name duration duedate',  // Ensure the correct fields are populated
      });
    if (!subscriber) {
    ...
    // Check if scheme is populated before accessing its fields
    const schemeDetails = subscriber.scheme
      ? {
        schemeName: subscriber.scheme.name,  // Access scheme name
        amount: subscriber.amount,
        subscribedDate: subscriber.subscribedDate,
        dueDate: subscriber.scheme.duedate, // Correct field name
        duration: subscriber.scheme.duration, // Correct field name
        status: subscriber.status,
      }
      : {
        schemeName: "Scheme not available",
        amount: subscriber.amount,
        subscribedDate: subscriber.subscribedDate,
        dueDate: null,
        duration: null,
